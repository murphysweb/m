...
import indexFoot from 'dvd-service-com-footer';
import wechatinvite from './wechatinvite.vue';
import layout from './layout.es6';
import {savebackData, getbackData, isTryShop} from '../../../../common/js/utils.es6';
...
export default {
  data() {
    return {
      ...
      contentData: localStorage.getItem('index_first_page') && JSON.parse(localStorage.getItem('index_first_page')),
      initCategory: 0,
      headData: {
        top: 1,
        head: {},
  ...
  computed: {
    ...
    userIntro() {
      if (this.contentData.userIntro) {
        return this.contentData.userIntro;
      } else {
        return null;
      }
    },
  ...
  mounted: function() {
    ...
    this.$nextTick(function() {
      ...
      $(window).scroll(function() {
        icon = that.$refs['newMemberIcon'];
        that.backTop = window.scrollY;
        savebackData(that);
        if (util.getDocumentScrollTop() >= 150 && icon) {
          icon.style.transform = 'translate3d(0, -50px, 0)';
        } else if (icon) {
  ...
  methods: {
    changStyle(json) {
      ...
      // 初始化header透明
      this.index_header_style.head = {background: 'none'};
      this.index_header_style.v_menu = {background: 'none'};
      ...
      if (json['8']) {
        ...
        this.index_header_style.top0 = {backgroundImage: 'url(' + json['8'].imageFilePath + ')'};
      }
      if (json['1']) {
        ...
        // 搜索框一般字
        this.index_header_style.shop_name = {color: '#' + json['1'].defaultTextColor.substr(2)};

        // 搜索框特殊字
        this.index_header_style.dav_base_red_color = {color: '#' + json['1'].specificTextColor.substr(2)};
      }
      if (json['2']) {

        // 搜索icon
        this.index_header_style.search_icon = {backgroundImage: 'url(' + json['2'].imageFilePath + ')'};
      }

      if (json['5']) {

        // 分类icon
        this.index_header_style.classification_icon = {backgroundImage: 'url(' + json['5'].imageFilePath + ')'};
      }
      if (json['6']) {

        // 购物车icon
        this.index_header_style.cart_icon = {backgroundImage: 'url(' + json['6'].imageFilePath + ')'};
      }
      if (json['7']) {
      ...
      if (json['9']) {
        ...
        this.index_header_style.hoverSpan = {borderBottom: '2px solid #' + json['9'].bottomLineColor.substr(2)};
        this.index_header_style.time_state_span_active = {color: '#' + json['9'].textSelectedColor.substr(2)};
      }
    },
    changeStyleByData(data) {
      ...
      data.map(function(item, index) {
        ...
        if (
          parseInt(startTime) <= parseInt(now) &&
          parseInt(endTime) > parseInt(now)
        ) {
          try {
    ...
    /* getSkinPackage() {
     ...
     let skinInfo = JSON.parse(localStorage.getItem('skinPackage'));
     this.changeStyleByData(skinInfo);
     }

     // 请求接口取到皮肤存到localStorage
     let obj = {elements: JSON.stringify(['skin'])};
     api('/api/mg/user/init/getInit', obj)
     .then(function (result) {
     if (result.code === 0) {
     let store = result.data.list[0].listData;
     ...
     text: 'code:' + result.code + ':msg' + result.data.msg,         // 文本（支持传入html。有则显示。）
     ...
     className: '',    // 钩子（支持传入class名。一个页面需要多种弹窗时，可以根据传入的className在页面设定各种样式）
     title: '',        // 标题（支持传入html。有则显示。）
     text: 'code:' + result.code,         // 文本（支持传入html。有则显示。）
     btnTitle: '',     // 按钮标题（支持传入html。有则显示，无则显示默认'确定'。）
     okBtnCallback() {   // 按钮点击回调（有则执行该回调）
    ...
    sessionHistory() {
      if (window.Units.isMobileIOS() || window.Units.isAndroid()) {
        if (
          ...
          JSON.parse(sessionStorage.getItem('history')).length > 1
        ) {
          if (
            ...
            JSON.parse(sessionStorage.getItem('history'))[
              JSON.parse(sessionStorage.getItem('history')).length - 2
            ].path
          ) {
    ...
    initHistory() {
      ...
      // let menuId = that.getQuery('menuId');
      this.initcate = that.getQuery('menuId');
      ...
      this.getPageFirst(!that.initcate ? true : false);
      this.cart();
      if (that.initcate) {
        that.channel(that.initcate);
      }
    ...
    init() {
      ...
      let start = null;
      if (
        sessionStorage.getItem('feedList') &&
        JSON.parse(sessionStorage.getItem('feedList')) &&
        JSON.parse(sessionStorage.getItem('feedList')).data &&
        this.queryPathType
      ) {
        this.contentData = JSON.parse(sessionStorage.getItem('feedList')).data;
      } else {
      ...
      if (this.queryPathType) {
        var strUrl = '/api/mg/sale/index/getPage';

        // var strData = layout.strSign('feed');
        var strData = encrypt.ajax({elements: JSON.stringify(['skin'])});
        ...
        success: function(data) {
          ...
          if (data.code == 0) {
            ...
            if (data.data) {
              ...
              sessionStorage.setItem('feedList', JSON.stringify(data));
              let feedIndex = 3;
              for (let i = 0; i < data.data.feedList.length; i++) {
                if (
                  data.data.feedList[i].body &&
                  data.data.feedList[i].body.tplId &&
                  data.data.feedList[i].body.tplId == 'bd_slide_0'
                ) {
                  feedIndex = i + 1;
                }
              }
              ...
              // }, 0);
              if (
                new Date().getTime() - new Date(2016, 11, 28).getTime() > 0 &&
                new Date().getTime() - new Date(2017, 0, 1).getTime() < 0
              ) {
              ...
              let data = JSON.parse(sessionStorage.getItem('feedList')).data;
              let feedIndex = 3;
              for (let i = 0; i < data.feedList.length; i++) {
                if (
                  data.feedList[i].body &&
                  data.feedList[i].body.tplId &&
                  data.feedList[i].body.tplId == 'bd_slide_0'
                ) {
                  feedIndex = i + 1;
                }
              }
    ...
    advert: function() {
      ...
      var that = this;
      if (
        new Date().getTime() - localStorage.getItem('advertTime') >=
        3600000
      ) {
        $.ajax({
          type: 'POST',
          ...
          success: function(data) {
            if (
              data.data_version ==
              JSON.parse(sessionStorage.getItem('dataVersion'))['advert'] ?
                JSON.parse(sessionStorage.getItem('dataVersion'))['advert'] :
                0
            ) {
              that.advert =
                localStorage.getItem('advertList') &&
                JSON.parse(localStorage.getItem('advertList')) &&
                JSON.parse(localStorage.getItem('advertList')).data ?
                  JSON.parse(localStorage.getItem('advertList')).data :
                  {};
            } else {
            ...
            if (that.advert.layer) {
              ...
              if (!localStorage.getItem('layerId')) {
                ...
                if (
                  localStorage.getItem('layerId') &&
                  JSON.parse(localStorage.getItem('layerId')) &&
                  JSON.parse(localStorage.getItem('layerId'))[that.date] &&
                  JSON.parse(localStorage.getItem('layerId'))[
                    that.date
                  ].indexOf(that.advert.layer.id) == -1
                ) {
                  that.advertFlag = [false, true, false];
                } else {
                  if (localStorage.getItem('layer') !== that.date) {
                    ...
                    if (
                      localStorage.getItem('topBarId') &&
                      JSON.parse(localStorage.getItem('topBarId')) &&
                      JSON.parse(localStorage.getItem('topBarId'))[that.date] &&
                      JSON.parse(localStorage.getItem('topBarId'))[
                        that.date
                      ].indexOf(that.advert.topBar.id) == -1
                    ) {
                      that.topBar();
                    } else {
                      if (
                        localStorage.getItem('topBar') !== that.date &&
                        that.advert.topBar
                      ) {
                        that.topBar();
                      }
    ...
    getPageFirst: function(acceptFeed) {
      var that = this;
      $.ajax({
        type: 'POST',
        url: '/api/mg/sale/index/getPage',
        data: encrypt.ajax({
        ...
        success: function(response) {
          if (!response.code) {
            ...
            // api/mg/sale/index/getPage
            (function() {
              if (acceptFeed) {
                that.contentData = response.data.pageFirst;
                localStorage.setItem('index_first_page', JSON.stringify(that.contentData));
              }
            })();

            // api/mg/sale/index/getSearch'
            (function() {
              that.headData.head = response.data.searchInfo;
              that.usersta = response.visitor_status;
              window.visitor_status = response.visitor_status;
            })();

            // api/mg/sale/index/getMenu
            (function() {
              that.menudata = response.data.menuInfo;
              that.getDataForUse(0);
              that.checkdownTip(response.visitor_status);
              response.data.menuInfo.shareInfo.shareSource = 2;
              share.setShareInfo(response.data.menuInfo.shareInfo, response);
            })();

            // api/mg/user/init/getInit
            (function() {
              let store = response.data.initInfo.list[0].listData;

              // 更改子组件的store数据，触发子组件的watch监听
              that.$children.map(function(item, index) {
                if (item.name == 'index') {
                  item.$children.map(function(item2, index2) {
                    if (item2.name == 'com-footer') {
                      item2.store = store;
            ...
            // api/mg/auth/inviter/checkAdviser
            (function() {
              that.$nextTick(function() {
                if (response.data.checkAdviserInfo.needPop) {
                  if (!+response.data.checkAdviserInfo.force) {
                  ...
                  // api/mg/user/advisergroup/indexPop
                } else {

                  // 如果没有妈妈顾问信息要弹出则请求判断是否要谈微信群邀请
                  let wechatalert = localStorage.getItem('wechatalert');
                  if (!wechatalert) {
                    if (response.data.indexPopInfo && response.data.indexPopInfo.needPop) {
                      localStorage['wechatalert'] = '1';
                      that.wechatdata = response.data.indexPopInfo;
            ...
            sessionStorage.setItem('share_seller_name', response.share_seller_name);
            window.scrollTo(0, 0);
          } else {

            // alert('/api/mg/sale/index/getPage为->', data.code);
          }
        },
        error: function(e) {
          console.log(e);
      ...
      $.ajax({
        ...
        success: function(data) {
          if (!data.code) {
            ...
            alert(
              '/api/mg/sale/channel/getPageFirst为->',
              data.code,
              'menId->',
              category
            );
     ...
     *  判断是否需要弹出微信群邀请
     */
    /* wechatGroupinvite: function () {
     let that = this;
     api('/api/mg/user/advisergroup/indexPop', {
     dataType: 'json',
     type: 'post'
     })
     .then(function (result) {
    ...
    // 关闭微信群邀请弹窗
    closeWechat(flag) {
      this.wechatFlag = flag;
    },
    changeCategory: function(category, index) {
      debugger;
      if (category == '-1') {
        window.location.href = this.menuList[
      ...
      if (index == 0) {
        ...
        /* if (
          layout.sStorageGet('index_first') &&
          layout.sStorageGet('index_first').data
        ) {
        ...
        $.ajax({
          type: 'POST',
          url: '/api/mg/sale/index/getPage',
          data: encrypt.ajax({elements: JSON.stringify(['skin'])}),
          success: function(data) {
            window[flag_name] = true;
            NProgress.done();
            if (!data.code) {
              that.contentData = data.data.pageFirst;
              let objData = {
                top: 0,
                data: that.contentData
              };
      ...
      debugger;
      var objApi = {menuId: category};

      /* if (
        layout.sStorageGet('first_' + category) &&
        layout.sStorageGet('first_' + category).data
      ) {
        ...
        success: function(data) {
          ...
          if (!data.code) {
            if (data.data.webUrl) {
              ...
              if (layout.sStorageGet('v_index', 'category')) {
                that.changeCategory(
                  layout.sStorageGet('v_index', 'category'),
                  layout.sStorageGet('v_index', 'index')
                );
              }
              location.href = data.data.webUrl;
            } else if (data.data.feedList) {
    ...
    /* initDate: function () {
     ...
     // 设置分享信息
     data.data.shareInfo.shareSource = 2;
    ...
    exit: function(event) {
      ...
      if (this.advert.topBar) {
        if (localStorage.getItem('topBarId')) {
          if (
            JSON.parse(localStorage.getItem('topBarId')) &&
            JSON.parse(localStorage.getItem('topBarId'))[this.date] &&
            JSON.parse(localStorage.getItem('topBarId'))[this.date].indexOf(
              this.advert.topBar.id
            ) == -1
          ) {
            this.topBar();
          } else {
            if (
              localStorage.getItem('topBar') !== this.date &&
              this.advert.topBar
            ) {
              this.topBar();
            }
    ...
    // 跳转新手会员引导页
    goMemberGuidePage() {
      $.post('/api/mg/user/intro/userClick?_=' + Date.now(), encrypt.ajax(), res => {
        if (!res.code) {
          location.href = this.userIntro.url;
        } else {
