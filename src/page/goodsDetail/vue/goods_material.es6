...
export default {
    ...
    return {
      ...
      items:[
        ...
        // 商品图片
        {
          ...
          //            img: 'http://9i.dvmama.com/supplier/2017/12/12/800_800_877bd186b4dadea40e9620b0e07c8dcf.jpg',
          x: 44.5,
          y: 14.5,
          width: 257,
          height:257
  ...
  components: {'com-swiper':comSwiper},
  watch: {},
  created() {
  ...
  methods: {
    ...
    choose() {
      ...
      if (this.activityinfo && this.activityinfo.activitys.length > 0) {
        ...
        this.items[8].img = '//9i.dvmama.com/free/2018/05/09/group_icon.png';
      } else {
    ...
    oneKeyShare() {
      ...
      $.ajax({
        ...
        type: 'post',
        data: encrypt.ajax({goodsId:parentId}),
        dataType: 'json',
        success(respones) {
          try {
        ...
        if (that.selectItem.imageList) {
          that.selectItem.imageList.forEach(function(imgObj, i, list) {
            if (imgObj.imgSplice.indexOf('speard.php') > -1) {
              var imagesObj = {host:window.location.host};
