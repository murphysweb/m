  ...
  <div class = "goods_bottom_wrapper">
    <div v-if = "!otheract.isStacks">
        ...
        <div class="goods_bottom">
            ...
            <div v-if = "!seckill" class = "w100">
                ...
                <div v-if = "!infoobj.presale">
                    <template v-if = "isbook == 2">
                      <template v-if = "infoobj.item && infoobj.item.typeId == '11' && !isOver">
                          <div v-if = "isclose == true"
                               ...
                               :dataid = "datarepresentid"
                               @click = "handleBuy($event)">一键购</div>
                          <div v-else
                            class = "btn_buy"
                            @click = "handleAddCartBuy">一键购</div>
                      </template>
                      <template v-else-if="(isbiggiftgoods||fromearn)&&isseller">
                            <template v-if="isbiggiftgoods">
                              <template v-if="userbuygift">
                                <div v-if = "isclose == true"
                                     class = "seller_buy"
                                     :dataid = "datarepresentid"
                                     @click = "handleBuy($event)">购买</div>
                                <div v-else
                                     class = "seller_buy"
                                     @click = "handleAddCartBuy">购买</div>
                              </template>
                                <div :class = "{seller_share: userbuygift, btn_buy: !userbuygift}"
                                @click = "giftShare">{{giftShareText}}</div>
                            </template>
                            <template  v-else>
                                <div v-if = "isclose == true"
                            class = "add_cart"
                        ...
                        <div class = "btn_buy" @click = "giftShare">分享赚钱</div>
                            </template>
                      </template>
                      <template v-else>
                        ...
                        <div v-else
                            ...
                            <template v-else>
                                <div v-if = "isclose == true"
                                    class = "btn_buy"
                                    :dataid = "datarepresentid"
                                    @click = "handleBuy($event)">使用抵用券￥368<del>￥398</del>购买大礼包</div>
                                <div v-else
                                    class = "btn_buy"
                                    @click = "handleAddCartBuy">立即购买</div>
                            </template>
                    ...
                    <template v-if="isbook == 1">
                      <div @click.stop="gobook" class="isbook">逛逛小书库书单</div>
                    </template>
                </div>
                <!-- 付定金 -->
                <div v-if = "isclose == true"
                    ...
                    :class = "{ disableGray: Number(goodstatus.goodsStocks) <= 0}"
                    :dataid = "datarepresentid"
                    @click = "handleBuy($event)">立即付定金</div>
                <div v-else
                class = "btn_buy"
                @click = "handleAddCartBuy">立即付定金</div>
                <!-- <div v-else
                    :class = "{ disableGray: Number(goodstatus.goodsStocks) <= 0}"
        ...
        <div class = "recommend_goods" v-if = 'mayyoulikelist && mayyoulikelist.length'>
            <div v-if = "goodstatusonsale == '1'">
            <div v-if = "Number(goodstatus.goodsStocks) <= 0&& (!ismultigoods || (ismultigoods&&allgoodsstocknumber==0))">
                ...
                <div class = "recommend_goods_cont">
                    ...
                    <scroller lock-y :scrollbar-x=false :bounce = "false">
                        <div class = "recommendList"  :style = "{width: mayyoulikelist.length * 130 + 'px'}">
                            <a class="other_goods_cont"
                                href = "javascript:void(0)"
                                @click = "handleGuess(item)"
                                v-for="item of mayyoulikelist">
                                <div class = "other_pic">
                                  <div class="imageUrl"><img v-lazy="item.imageUrl"></div>
                                </div>
    ...
    <div class = "goods_stacks_bottom" v-else ref = "selects">
        <div class = "goods_bottom_back" @click = "handleBookBack">返回</div>
        <!--售罄和未上架显示看看别的-->
        <div class = "recommend_goods" v-if = 'mayyoulikelist && mayyoulikelist.length'>
            <div v-if = "goodstatusonsale == '1'">
            <div v-if = "Number(goodstatus.goodsStocks) <= 0">
...
<style lang="scss" scoped>
  ...
  .seller_buy {
    background: linear-gradient(to right, #FFB21A, #FF7C31);
    float:left;
    /*line-height: 18px;*/
    /*padding-top: 8px;*/
    /*height:49px;*/
