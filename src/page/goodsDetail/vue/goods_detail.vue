  ...
  <div v-else :class="[{'big_gift':isBigGiftGoods,'from_earn':fromEarn}]">
    ...
    <div v-else>
      ...
      <div>
        <div
          class="goodsWrapper"
          :class="{ pciTabMtopT: isPrompt && !isApp && !isBigGiftGoods}"
        >
          <goods-swiper
            :albuminfo = "albuminfo"
            :isapp="isApp"
          ...
          <goods-intro
            ...
            :otheract="otherAct"
            :response="response"
            :goodsdatabasis="goodsDataBasis"
            :goodsparamobj="goodsParamObj"></goods-intro>
          <!--广告banner-->
          <ad-banner :ad-img="response.data.ADBanner.imageUrl" :ad-url="response.data.ADBanner.content"
                     v-if="response && response.data && response.data.ADBanner && response.data.ADBanner.imageUrl&&!isBigGiftGoods">
          </ad-banner>
          <!-- 赚钱养家专用图  -->
          <div class="zhuanqian-box" v-if="isBigGiftGoods">
            <h5 class="zhuanqian-title">购买<span>礼包</span>后将获得:</h5>
          ...
          <activity-types
            ...
            :otheract="otherAct"
            :goodsstocknumber="goodsStockNumber"
            :allgoodsstocknumber='allGoodsStockNumber'
          ...
          <!--商品素材-->
          <goods_material v-if="infoObj.price && !isBigGiftGoods" :goodsdatabasis="goodsDataBasis"
                          :materials="materials"
                          :price="infoObj.price"
                          :goodsid="goodsId"
                          ...
                          :otheract="otherAct"
                          :activityinfo="activityInfo"
                          @ievent = "ievent"></goods_material>

          <goods-evaluate
            v-if="firstScreenFinish&& !isBigGiftGoods"
            ...
            <div
              ...
              <div class="parames clearfix" v-show="detailIndex == 0">
                <detail-pic
                  ...
                  :addmodulemes = "addModuleMes"></detail-pic>
              </div>
              <div class="picCont"
                   v-if="goodsParamObj.length"
      ...
      <goods-bottom
        ...
        :seckill="secKill"
        :otheract="otherAct"
        :isbook="isVipBookGoods"
        :goodsstocknumber="goodsStockNumber"
        :allgoodsstocknumber='allGoodsStockNumber'
        :isbiggiftgoods="isBigGiftGoods"
        :fromearn="fromEarn"
        :isseller="isSeller"
        :position="position"
        :userbuygift="userBuyGift"
