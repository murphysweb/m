  ...
  methods: {
    ...
    GetUrlPara(){
      ...
      let arrurl= url.split("//"),
        ...
        relurl = arrurl[1].substring(start).split("/");
      if (relurl[1].indexOf(".")!== -1 ) {
        this.goodsId = relurl[1].split(".")[0];
      }
      return this.goodsId;
    },
    //静态化数据
    ...
    getAddModule() {
      ...
      $.ajax({
        ...
        type: 'post',
        dataType: 'json',
        data: encrypt.ajax({goodsId: goodsId}),
        success(response) {
    ...
    getData(querys) {
      ...
      let getDetail = new Promise(function (resolve, reject) {$.ajax({
        type: type,
        url: detailURL,
