<template>
	<div class = "activitys" v-if = "!otheract.isStacks">
		<group class = "goodsInfos clearfix" v-if="!isbiggift">
		...
		<group v-if = "activityinfo.activitys && activityinfo.activitys.length &&!isbiggift" class = "goodsInfos clearfix activity_cont">
			<!--如果1个活动跳连接，两个弹框-->
			<cell-box
		...
		<group v-if = "activityinfo.notice && activityinfo.notice.noticeName&&!isbiggift" class = "goodsInfos">
			<cell-box
					is-link
		...
		<!-- <group v-if = " isbiggift && bigGiftParmas=='938517' " class = "goodsInfos">
			<cell-box
					is-link
					:link = "activityinfo.notice.command.content"
					class = "activity_t_cont">
				<div  class = "activity_t_title">活动</div>
				<div class = "activity_t_common">
					<span class = "new_flash">{{activityinfo.notice.noticeName}}</span>
				</div>
			</cell-box>
		</group> -->
		<!-- 大礼包商品id为938517显示活动区域，为了解决串连商品notice为空，特殊处理 -->
		<template v-if="isbiggift && (goodsid=='938517' || goodsid == '952548') && (activityinfo.notice.noticeId=='371' || activityinfo.notice.noticeId=='383') ">
			   <group class = "goodsInfos">
			    <cell-box
			      is-link
			      :link = "activityinfo.notice.command.content"
			      class = "activity_t_cont">
		...
		<group class = "goodsInfos servicesInfo" @click.native = "handleServer" >
			<cell-box class = "activity_t_cont clearfix servicePad" is-link>
				...
				<div class = "activity_t_common clearfix serviceWid" v-if = "activityinfo.service && !isbiggift">
					<div class = "clearfix">
					...
					<div class = "clearfix">
					  <span class = "service_s" :class="{ 'removeStyle':index==2, servicesStyle:index == 1}" v-for = "(item,index) of activityinfo.service.slice(3)">
						 <span :class="{'service_m_o':index == 0,'third_width': index==2}">
							<span class = "service_icon"></span>
							<span v-if="index!=2" v-text="item.serviceTitle1"></span>
							<span v-if="index==2" v-text="item.servicePolicy"></span>
						  </span>
						  <span v-if="index==2" style="width:20%;vertical-align: top;">不发货</span>
					  </span>
					</div>
				</div>
				<div class = "activity_t_common clearfix serviceWid" v-if = "isbiggift">
					<div class = "clearfix">
					  <span class = "service_s" :class="{ 'removeStyle':index==5,'giftStyle':index==5}" v-for = "(item,index) of activityinfo.service" v-if="item.isEarningGoods">
						 <span :class="{'service_m_o':index == 0,'third_width': index==5}">
							<span class = "service_icon"></span>
							<span v-if="index!=5" v-text="item.serviceTitle1"></span>
							<span v-if="index==5" v-text="item.servicePolicy"></span>
						  </span>
						  <span v-if="index==5" style="width:20%;vertical-align: top;">不发货</span>
					  </span>
					</div>
				</div>
			</cell-box>
		...
		<popup  v-model="serverShow"
			...
			<div class = " server_scroll">
			  ...
			  <div class = "server_title_scroll">
				<group v-for = "item of activityinfo.service" v-if="!isbiggift || (isbiggift&&item.isEarningGoods)">
					<div @click = "handleServices(item)">
						<cell-box
								:is-link = "!!item.command.content || !!item.servicePolicyRtf"
								class = "server_h">
						  <i class = "service_t_icon"></i><span class = "services_title">{{ item.serviceTitle2 }}</span>
						</cell-box>
						<div class = "server_c" v-if="item.serviceTitle2 == '不支持无理由退换货'">签收商品之日起15天内，如有质量问题可申请退换货</div>
						<div class = "server_c" v-else>{{ item.servicePolicy }}</div>
					</div>
				</group>
			  </div>
			</div>
				...
				<div class="summary modalPicCont">
				  ...
				  <div class = "titleInfo">
					...
					<div class = "summary_select"><span class = "summar_select_title">选择</span>
					  ...
					  <div class = "summary_m15">
						  <span v-for = "item of goodsmodalobj.goodsType" v-if = "goodstags.length">
							  {{ item.title }}
						  </span>
					  </div>
					</div>
				...
				<div class="sku-control">
					...
					<div class="summary_number">
						...
						<div class = "summary_number_cont">
						  <div v-if = "infoobj.presale && cartNum == 200" class = "preIsLimit">限购{{ infoobj.limitNum }}件</div>
				  ...
				  <div class = "activeBottomBtn" v-if = "goodstatusonsale != '0'">
					...
					<div v-else>
					  ...
					  <div v-else>
						...
						<div v-else>
						  <!-- <template v-if = "infoobj.item && infoobj.item.typeId == '0'">
						  ...
						  <template v-if = "infoobj.item && infoobj.item.typeId == '11' && !isOver">
							<div
								class = "btn_buy btn_buy_width"
								:class = "{ disableGray: Number(goodstatus.goodsStocks) <= 0 }"
								:dataid = "datarepresentid"
						  ...
						  <template v-else>
							...
							<!-- <div class="cart_buy_b"
								...
								:isclose = "isclose"
								@click = "handleModalCart($event)">立即购买</div> -->
								<template v-if="this.isbiggift==false && this.isSeller==false">
									<div class="add_cart_b"
									:dataid = "datarepresentid"
									:isclose = "isclose"
									@click = "handleModalConfirm($event)">加入购物车</div>
								...
								<template v-if="this.isbiggift==false && this.isSeller==true">
									<div class="add_cart_b"
									:dataid = "datarepresentid"
									:isclose = "isclose"
									@click = "handleModalConfirm($event)">加入购物车</div>
								...
								<template v-if="this.isbiggift==true && this.isSeller==false">
									<div class="cart_buy_b"
									:dataid = "datarepresentid"
									:isclose = "isclose"
									@click = "handleModalCart($event)">立即购买</div>
								</template>
								<template v-if="this.isbiggift==true && this.isSeller==true">
									<div class = "btn_buy"
										@click = "giftShare" v-if="!userbuygift">去分享</div>
                  <div class="cart_buy_b"
                       :dataid = "datarepresentid"
                       :isclose = "isclose"
                       @click = "handleModalCart($event)" v-else>立即购买</div>
								</template>
						  </template>
						</div>
					  </div>
		...
		<div class = "dialogCont">
			<div class = "dialogHead" @click.stop= "handleBack"><span class = "home_arrow"></span></div>
