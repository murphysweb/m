...
const config = {
  ...
  defaultShareInfo: {
    title: 'MAMA+|大V店', // 分享标题
    desc: 'MAMA+|大V店', // 分享描述
    link: runtime.isClient() ? window.location.href : 'https://bravetime.davdian.com', // 分享链接
...
function getProtocol(options = {}) {
  ...
  if (runtime.isClient()) {
    ...
    window[callbackName] = function (response) {
      ...
      if (response.code == '1') {
        if (options.param && options.param.success)
          options.param.success(response);
    ...
    if (options.param.shareCallBack) {
      let shareCb = options.param.shareCallBack;
      let shareCbName = `native_share_callback_${Math.random().toString().split('.')[1]}`;
  ...
  // 拼接唤起native协议
  let protocol = `davdian:\/\/call.${options.host}.com?action=${options.action}&params=${encodeURIComponent(JSON.stringify(options.param))}&callback=${callbackName}&minv=${options.v}`;
  return protocol;
}
...
function innerCall(param = {}) {
  ...
  if (ua.compareVersion(ua.getDvdAppVersion(), param.v) >= 0) {
    // 客户端端内调用协议
    let protocol = getProtocol(param);
 ...
 */
function outerCall(param = {}, stayCurrentPage = false) {
  // 在app中,直接返回
  if (ua.isDvdApp()) return;
  ...
  if (ua.isAndroid()) {
    // 端外唤起协议
    let outerProtocol = `davdian://invoke.davdian.com?cmd=${encodeURIComponent(protocol)}`;
    ...
    // 端外唤起协议
    let outerProtocol = `//invoke.davdian.com?cmd=${encodeURIComponent(protocol)}`;
 ...
 */
function call(param = {}, stayCurrentPage) {
