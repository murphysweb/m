...
module.exports = {
  /**
   * 获取大V店app当前环境
   * @param {{}} options 指定二维码生成方式
   * @param {Function} cb 生成二维码时的回调
   * @returns {Promise} data为base64格式图片数据
   * @demo 请看下面↓
  ...
  getBase64(options) {
    return new Promise((resolve, reject) => {
      ...
      if (settings.logo) {
        ...
        settings.image.crossOrigin = 'anonymous';
        settings.image.onload = function () {
          var base64 = createCanvas(settings)[0].toDataURL('image/png');
          resolve(base64);
        };
        settings.image.onerror = function () {
          reject(`qrcode getBase64 logo图片加载失败：${settings.logo}`);
        };
        settings.image.src = settings.logo;
      } else {
        // 如果没有图片，立即回调
        var base64 = createCanvas(settings)[0].toDataURL('image/png');
        resolve(base64);
